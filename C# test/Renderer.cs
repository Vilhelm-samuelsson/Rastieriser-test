...
namespace MainEngine.Rendering
{
    public static class ImageRenderer
    {
        ...
        {
            Vector3 vertexworld = transform.ToWorldPoint(point);
            Vector3 vertexcamera = camera.transform.ToLocalPoint(vertexworld);

            float screenheight_world = MathF.Tan(camera.fov / 2) * 2;
            float pixelsperworldunit = size.Y / screenheight_world / vertexcamera.Z;

            Vector2 pixeloffset = new Vector2(vertexcamera.X, vertexcamera.Y) * pixelsperworldunit;
            return new Vector3(size / 2 + pixeloffset, vertexcamera.Z);
        }
        ...
        public static void RenderImage(Rendertarget rendertarget, Scene scene)
        {
            ...
            for (int o = 0; o < scene.gameobjects.Count; o++)
            {
                ...
                Parallel.For(0, indecis.Length / 3, Iindex =>
                {
                    int triangleindex = Iindex * 3;
                    Vector3 a3 = TransformToscreenSpace(Verts[indecis[triangleindex]], rendertarget.size, objecttorender.transform, scene.camera);
                    Vector3 b3 = TransformToscreenSpace(Verts[indecis[triangleindex + 1]], rendertarget.size, objecttorender.transform, scene.camera);
                    Vector3 c3 = TransformToscreenSpace(Verts[indecis[triangleindex + 2]], rendertarget.size, objecttorender.transform, scene.camera);
                    ...
                    if (a3.Z >= 0 && b3.Z >= 0 && c3.Z >= 0)
                    {
                        ...
                        for (int y = blockstarty; y <= blockendy; y++)
                        {
                            for (int x = blockstartx; x <= blockendx; x++)
                            {
                                ...
                                {
                                    Vector3 deapths = new Vector3(a3.Z, b3.Z, c3.Z);
                                    float deapth = 1 / Helper.dot(Helper.oneminus(deapths), weights);
                                    if (deapth >= rendertarget.DeapthBuffer[pixelindex]) continue;

                                    Vector3 Normal = Vector3.Zero;
                                    if (!objecttorender.mesh.shader.smoothnormals)
                                    {
                                        Normal += objecttorender.mesh.normals[Math.Clamp(Iindex, 0, objecttorender.mesh.normals.Length - 1)];
                                    }
                                    else
                                    {
                                        Normal += objecttorender.mesh.normals[Math.Clamp(indecis[triangleindex + 0], 0, objecttorender.mesh.normals.Length - 1)] / deapths[0] * weights[0];
                                        Normal += objecttorender.mesh.normals[Math.Clamp(indecis[triangleindex + 1], 0, objecttorender.mesh.normals.Length - 1)] / deapths[1] * weights[1];
                                        Normal += objecttorender.mesh.normals[Math.Clamp(indecis[triangleindex + 2], 0, objecttorender.mesh.normals.Length - 1)] / deapths[2] * weights[2];
                                        Normal *= deapth;
                                    }
                                    ...
                                    Vector2 texcord = Vector2.Zero;
                                    texcord += objecttorender.mesh.UVS[Math.Clamp(indecis[triangleindex + 0],0, objecttorender.mesh.UVS.Length - 1)] / deapths[0] * weights[0];
                                    texcord += objecttorender.mesh.UVS[Math.Clamp(indecis[triangleindex + 1],0, objecttorender.mesh.UVS.Length - 1)] / deapths[1] * weights[1];
                                    texcord += objecttorender.mesh.UVS[Math.Clamp(indecis[triangleindex + 2],0, objecttorender.mesh.UVS.Length - 1)] / deapths[2] * weights[2];
                                    texcord *= deapth;

                                    lock (rendertarget.locks[pixelindex])
                                    {
                                        rendertarget.ColorBuffer[pixelindex] = objecttorender.mesh.shader.pixelcolor(deapth, Normal, texcord);
